...
namespace Nop.Services.NhaXes
{
    public class PhoiVeService : IPhoiVeService
    {
        ...
        private readonly IRepository<HanhTrinhGiaVeThoiGian> _hanhtrinhgiavethoigianRepository;
        ...
        public PhoiVeService(ICacheManager cacheManager,
            ...
            IRepository<HanhTrinhGiaVeThoiGian> hanhtrinhgiavethoigianRepository,
            ...
            )
        {
            ...
            this._hanhtrinhgiavethoigianRepository = hanhtrinhgiavethoigianRepository;
        ...
        public virtual string ThanhToanGiaoVe(PhoiVe item)
        {
            item.NgayUpd = DateTime.Now;
            item.TrangThai = ENTrangThaiPhoiVe.DaGiaoHang;
            var ngayhientai = DateTime.Now;
            var chuyendi = _hisRepository.GetById(item.ChuyenDiId);
            var menhgia = _menhgiaveRepository.Table.Where(c => c.MenhGia == item.GiaVeHienTai).FirstOrDefault();
            var chang = _hanhtrinhgiavethoigianRepository.GetById(item.ChangId);
           // var giaodichkevemenhgiaids = _giaodichkevemenhgiaRepository.Table.Where(c => c.giaodichkeve.TrangThaiId != (int)ENGiaoDichKeVeTrangThai.HUY && c.NguoiNhanId == item.NguoiDatVeId && c.QuanLyMauVeKyHieuId != null && c.ChangId == item.ChangId).Select(c => c.Id).ToList();
           // var vexeitemids = _giaodichkevexeitemRepository.Table.Where(c => giaodichkevemenhgiaids.Contains(c.GiaoDichKeVeMenhGiaId)).Select(c => c.VeXeItemId).ToList();
            var query = _vexeitemRepository.Table
                        .Where(c => c.MenhGiaId == menhgia.Id && c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_GIAO_HANG
                            && c.NhanVienId == _workContext.CurrentNhanVien.Id && c.HanhTrinhId == chang.HanhTrinh.Id && c.ChangId==chang.Id);
            if (ngayhientai.Date == chuyendi.NgayDi.Date)
            {
                query = query.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop);

            }
            if (ngayhientai.Date < chuyendi.NgayDi.Date)
            {
                query = query.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeBanTruoc || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop);
            }
            query = query.OrderBy(c => c.SoSeriNum);
            if (query.Count() == 0)
             return "KhongCoSeri";
            var serive = query.First();
            if (query != null)
            {
        ...
        public virtual string GanSeriVeBoSung(PhoiVe item)
        {
            var chuyendi = _hisRepository.GetById(item.ChuyenDiId);
            var query = _vexeitemRepository.Table.Where(c => c.NhaXeId == _workContext.NhaXeId && !c.isDeleted
                //&& !(c.VanPhongId > 0)
                && (c.VanPhongId > 0)
                && (c.isVeBoSung)
                && c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_SU_DUNG
                && c.XeXuatBenId == chuyendi.Id
            ...
            {
                //cap nhật serive
                serive.TrangThai = ENVeXeItemTrangThai.DA_GAN_SERI_BOSUNG;
                serive.XeXuatBenId = chuyendi.Id;
                serive.NguonVeXeId = item.NguonVeXeId;
                serive.NgayBan = item.NgayDi;
                serive.ChangId = item.ChangId;
                serive.isGiamGia = item.IsForKid;
        ...
        public virtual List<NhaXeCustomer> GetKhachHangInNhaXe(string SearchKhachhang, int nhaxeid)
        {
            ...
            if (nhaxeid > 0)
                  query = query.Where(m => m.NhaXeId == nhaxeid && m.DienThoai.Contains(SearchKhachhang));
            return query.ToList();
        }
        ...
        public virtual string NhaXeThanhToanGiuChoPhoiVeTheoChuyen(int NhaXeId, int ChuyenDiId, int ChangId, string SessionId, int NguoiDatVeId, bool DaThanhToan,int DaiLyId, int CustomerId, string GhiChu,string ViTriLen,string ViTriXuong, bool IsForKid)
        {
            //var chang = _hanhtrinhgiaveRepository.GetById(ChangId);
            var chuyendi = _hisRepository.GetById(ChuyenDiId);
            var ngayhientai = DateTime.Now;
            var chang = _hanhtrinhgiavethoigianRepository.GetById(ChangId);
            ...
            var pv = items.First();
            //var giaodichkevemenhgiaids = _giaodichkevemenhgiaRepository.Table.Where(c => c.giaodichkeve.TrangThaiId != (int)ENGiaoDichKeVeTrangThai.HUY && c.NguoiNhanId == pv.NguoiDatVeId && c.QuanLyMauVeKyHieuId != null && c.ChangId == ChangId).Select(c => c.Id).ToList();
           // var vexeitemids = _giaodichkevexeitemRepository.Table.Where(c => giaodichkevemenhgiaids.Contains(c.GiaoDichKeVeMenhGiaId)).Select(c => c.VeXeItemId).ToList();
            if (items.Count() == 0)
                return "KhongTimThayCho";
            if(DaThanhToan)
            {               
                if (pv != null)
                {
                    ...
                    if (mg == null)
                        return "KhongMenhGia";
                    //var mg = _menhgiaveRepository.Table.Where(c => c.MenhGia == chang.GiaVe).ToList();
                    //if (mg.Count() == 0)
                    //  return "KhongMenhGia";
                    //var mgids = mg.Select(c => c.Id).ToArray();
                    
                    var query = _vexeitemRepository.Table
                       .Where(c => c.MenhGiaId == mg.Id && c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_GIAO_HANG
                           && c.NhanVienId == pv.NguoiDatVeId && c.HanhTrinhId == chang.HanhTrinh.Id && c.ChangId==ChangId).OrderBy(c => c.SoSeriNum);
                    if (ngayhientai.Date == chuyendi.NgayDi.Date)
                    {
                        var soveitem = query.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop).Count();
                        if (items.Count() > soveitem)
                            return "KhongDuVeDiNgay";
                    }
                    if (ngayhientai.Date < chuyendi.NgayDi.Date)
                    {
                        var soveitem = query.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeBanTruoc || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop).Count();
                        if (items.Count() > soveitem)
                            return "KhongDuVeBanTruoc";
                    }
                    if (ngayhientai.Date > chuyendi.NgayDi.Date)
                        return "QuaHanDatVe";
                    //if (items.Count() > soveitem)
            ...
            foreach (var item in items)
            {
                ...
                if (DaThanhToan)
                {
                    ...
                    var vexeitem = _vexeitemRepository.Table
                        .Where(c => c.MenhGiaId == menhgia.Id && c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_GIAO_HANG
                            && c.NhanVienId == item.NguoiDatVeId && c.HanhTrinhId == chang.HanhTrinh.Id && c.ChangId==ChangId);
                    if (ngayhientai.Date == chuyendi.NgayDi.Date)
                    {
                        vexeitem = vexeitem.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop);
                    }
                    if (ngayhientai.Date < chuyendi.NgayDi.Date)
                    {
                        vexeitem = vexeitem.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeBanTruoc || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop);
                    }
                    vexeitem = vexeitem.OrderBy(c => c.SoSeriNum);
                    if (vexeitem.Count() == 0)
                        return "KhongCoSeri";
        ...
        public virtual string NhaXeThanhToanNhanhTheoChuyen(int ChuyenDiId, int changid, string SessionId, int NguoiDatVeId)
        {
           ...
           // var vexeitemids = _giaodichkevexeitemRepository.Table.Where(c => giaodichkevemenhgiaids.Contains(c.GiaoDichKeVeMenhGiaId)).Select(c => c.VeXeItemId).ToList();
            if (pv != null)
            {
                ...
                var query = _vexeitemRepository.Table
                   .Where(c => c.MenhGiaId == mg.Id && c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_GIAO_HANG
                       && c.NhanVienId == NguoiDatVeId && c.HanhTrinhId == chang.HanhTrinh.Id && c.ChangId==changid);
                if (ngayhientai.Date == chuyendi.NgayDi.Date)
                {
                    var soveitem = query.Where(c => c.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay || c.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop).Count();
                    if (items.Count() > soveitem)
        ...
        public List<KhachHangMuaVeItem> GetKhachHangMuaVeTheoTuyen(int nhaxeid, DateTime tuNgay, DateTime denNgay, List<int> LichtrinhIds, int KhachHangId = 0)
        {
           ...
           .ToList();*/
            if (KhachHangId == 0)
            {
                var phoives = _phoiveRepository.Table.Where(c=>LichtrinhIds.Contains(c.nguonvexe.LichTrinhId)
                    && (c.TrangThaiId ==(int)ENTrangThaiPhoiVe.DaGiaoHang || c.TrangThaiId==(int)ENTrangThaiPhoiVe.ChoXuLy)
                    && (c.NgayDi>=tuNgay)
                    && (c.NgayDi<=denNgay))
                 .GroupBy(gb => new { gb.customer })
           .Select(g => new KhachHangMuaVeItem
           {
               CustomerId = g.Key.customer.Id,
               customer = g.Key.customer,
        ...
        public List<DoanhThuTheoXeItem> GetDoanhThuBanVeTungXeTheoNgay(DateTime tuNgay, DateTime denNgay, int nhaxeid, int XeId)
        {
            var phoives = _phoiveRepository.Table
              ...
              .Where(c => c.NguonVeXe.NhaXeId == nhaxeid && c.HisXeXuatBen.XeVanChuyenId == XeId
                   && (c.PhoiVe.TrangThaiId == (int)ENTrangThaiPhoiVe.DaGiaoHang)
                   && c.PhoiVe.isChonVe == false
                   && (c.PhoiVe.NgayDi <= denNgay)
                   && (c.PhoiVe.NgayDi >= tuNgay)).ToList()
            .Select(c => new
            {
                NguonVeXeId = c.PhoiVe.NguonVeXeId,
                XeId = c.HisXeXuatBen.XeVanChuyenId,
                GiaVe = c.PhoiVe.GiaVeHienTai,
                NgayDi = c.PhoiVe.NgayDi.Date,
                TrangThaiPhoiVeId = c.PhoiVe.TrangThaiId
            })
            .GroupBy(c => new { c.NgayDi })
            .Select(g => new DoanhThuTheoXeItem
        ...
        public List<PhoiVe> GetPhoiVeByVanPhongId(int VanPhongId,DateTime NgayBan)
        {
            var query= _phoiveRepository.Table.Where(c=> c.TrangThaiId==(int)ENTrangThaiPhoiVe.DaGiaoHang
                && c.NgayDi.Year == NgayBan.Year
                && c.NgayDi.Month == NgayBan.Month
                && c.NgayDi.Day == NgayBan.Day);
            if (VanPhongId > 0)
                query = query.Where(c => c.nguoidatve.VanPhongID == VanPhongId);
            return query.ToList();
        }
        public List<ThongKeItem> GetDoanhThuBanVeTheoNgay(DateTime tuNgay, DateTime denNgay, int nhaxeid, int VanPhongId)
        {
            ...
            return tknhanvien;*/
            var phoives = _phoiveRepository.Table
                .Where(c=>c.nguoidatve.VanPhongID==VanPhongId
                && (c.TrangThaiId == (int)ENTrangThaiPhoiVe.DaGiaoHang || c.TrangThaiId==(int)ENTrangThaiPhoiVe.Huy)
                && c.VeXeItemId>0
                    && c.isChonVe == false
                    && (c.NgayDi <= denNgay)
             ...
             .Select(c => new
             {
                 NhanVienId = c.NguoiDatVeId.Value,
                 GiaVe = c.GiaVeHienTai,
                 NgayDi = c.NgayDi.Date,
                 TrangThaiPhoiVeId = c.TrangThaiId,
             ...
             {
                 ItemDataDate = g.Key.NgayDi,
                 GiaTri = g.Where(c=>c.TrangThaiPhoiVeId==(int)ENTrangThaiPhoiVe.DaGiaoHang).Sum(a => a.GiaVe),
                 SoLuong = g.Count(),
                 SoLuongDat = g.Count(a=>a.TrangThaiPhoiVeId !=(int)ENTrangThaiPhoiVe.Huy),
        ...
        public List<ThongKeItem> GetDoanhThuBanVeTheoNhanVien(int nhaxeid, int VanPhongId, DateTime NgayBan)
        {
            ...
            return tknhanvien;*/
            var query = _phoiveRepository.Table
               .Where(c => (c.TrangThaiId == (int)ENTrangThaiPhoiVe.DaGiaoHang || c.TrangThaiId == (int)ENTrangThaiPhoiVe.Huy)
                   && c.VeXeItemId>0
                    && c.isChonVe == false && c.NguoiDatVeId.HasValue
                    && (c.NgayDi.Year == NgayBan.Year)
                     && (c.NgayDi.Month == NgayBan.Month)
                     && (c.NgayDi.Day == NgayBan.Day));
             //lay tat ca cac nhan vien tham gia vao giao dich dat ve, chuyen ve, huy ve
            var nhanvienids = query.Select(c => c.NguoiDatVeId).Distinct().ToList();
            nhanvienids.AddRange(query.Where(c => c.NguoiHuyId > 0).Select(c => c.NguoiHuyId).Distinct().ToList());
            nhanvienids.AddRange(query.Where(c => c.NguoiChuyenId > 0).Select(c => c.NguoiChuyenId).Distinct().ToList());
            ...
            foreach(var item in listNhanVien)
            {
                ...
                nhanvien.SoLuongDat = query.Where(c => c.NguoiDatVeId == item.Id && c.TrangThaiId != (int)ENTrangThaiPhoiVe.Huy).Count();
                ...
                var _giatri = query.Where(c => c.NguoiDatVeId == item.Id && c.TrangThaiId != (int)ENTrangThaiPhoiVe.Huy).Select(g => g.GiaVeHienTai).ToList();
                if (_giatri.Count() > 0)
        ...
        public List<ThongKeItem> GetLoaiVeSeriByNhanVienId(int NhanVienId,DateTime NgayBan)
        {
            var query = _phoiveRepository.Table.Where(c => c.TrangThaiId == (int)ENTrangThaiPhoiVe.DaGiaoHang
                        && (c.NguoiDatVeId==NhanVienId)
                        && (c.NgayDi.Year == NgayBan.Year)
                        && (c.NgayDi.Month == NgayBan.Month)
                        && (c.NgayDi.Day == NgayBan.Day));
            var phoives = query.ToList();
            var tkphoive = new List<ThongKeItem>();
            foreach(var item in phoives)
            {
                var phoive = new ThongKeItem();
                if(item.vexeitem !=null)
                {
                    if (item.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeBanTruoc)
                        phoive.LoaiVe = "Vé bán trước";
                    if (item.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay)
                        phoive.LoaiVe = "Vé đi ngay";
                    if (item.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeTongHop)
                        phoive.LoaiVe = "Vé tổng hợp";
                    phoive.Seri = item.vexeitem.SoSeri;
                    tkphoive.Add(phoive);
                }
            }
            return tkphoive;
        }
        public List<ThongKeItem> GetLoaiVeSeriByXeId(int XeId, DateTime NgayBan)
        {
            var chuyendiids = _hisRepository.Table.Where(c => c.xevanchuyen.Id == XeId
                        && (c.NgayDi.Year == NgayBan.Year)
                        && (c.NgayDi.Month == NgayBan.Month)
                        && (c.NgayDi.Day == NgayBan.Day)).Select(c => c.Id).ToArray();
            var query = _phoiveRepository.Table.Where(c=>chuyendiids.Contains(c.ChuyenDiId));
            var phoives = query.ToList();
            var tkphoive = new List<ThongKeItem>();
            foreach (var item in phoives)
            {
        ...
        public List<DoanhThuHangNgay> GetDoanhThuHangNgay(DateTime TuNgay, DateTime DenNgay, int HanhTrinhId)
        {
            ...
            foreach (var chuyen in _items)
            {
                ...
                item.DTVeBanNgay = vexe.Where(c=>c.LoaiVeId==(int)ENLoaiVeXeItem.VeDiNgay).ToList().Sum(c=>c.GiaVe) +vexe.Where(c=>c.LoaiVeId==(int)ENLoaiVeXeItem.VeTongHop).Sum(c=>c.GiaVe);
        ...
        public List<LichSuBanVe> GetLichSuBanVeByChuyenDiId(int ChuyenDiId)
        {
            var query = _phoiveRepository.Table.Where(c => c.ChuyenDiId == ChuyenDiId && (c.TrangThaiId == (int)ENTrangThaiPhoiVe.DaGiaoHang || c.TrangThaiId == (int)ENTrangThaiPhoiVe.ChoXuLy || c.TrangThaiId == (int)ENTrangThaiPhoiVe.Huy));
            var nhanvienids = query.Select(c => c.NguoiDatVeId).Distinct().ToList();
            nhanvienids.AddRange(query.Where(c => c.NguoiChuyenId > 0).Select(c => c.NguoiChuyenId).Distinct().ToList());
            nhanvienids.AddRange(query.Where(c => c.NguoiHuyId > 0).Select(c => c.NguoiHuyId).ToList());
            ...
            foreach (var nhanvien in listNhanVien)
            {
                ...
                item.SoLuongDat = phoives.Where(c => c.NguoiDatVeId == nhanvien.Id && c.TrangThaiId != (int)ENTrangThaiPhoiVe.Huy).Count();
