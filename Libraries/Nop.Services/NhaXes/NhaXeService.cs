...
namespace Nop.Services.NhaXes
{
    public partial class NhaXeService : INhaXeService
    {
        ...
        private readonly IRepository<DaiLy> _dailyRepository;
        ...
        public NhaXeService(IRepository<ChotKhach> chotkhachRepository,
            ...
            IRepository<DaiLy> dailyRepository,
        ...
        public virtual List<HistoryXeXuatBen> GetChuyenDiByXeVanChuyenId(int XeId,DateTime NgayDi)
        {
            var query = _historyxexuatbenRepository.Table.Where(c => c.xevanchuyen.Id == XeId
                && c.NgayDi.Year == NgayDi.Year
                && c.NgayDi.Month == NgayDi.Month
            ...
            if (ThangId > 0 && NamId > 0)
            {
                ...
                .Where(c => (c.TrangThaiId == (int)ENTrangThaiXeXuatBen.DANG_DI
                    ...
                    ).ToList().Select(m =>
                    {
                        ...
                        item.Thang = ThangId;
                        var pvs= GetPhoiVeByChuyenDi(m.Id).Where(c=>c.vexeitem != null).ToList();
                        item.DoanhThu = pvs.Sum(c => c.GiaVeHienTai);
                        item.DoanhThuVeBanTruoc = pvs.Where(c => c.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeBanTruoc).Sum(c => c.GiaVeHienTai);
                        item.DoanhThuVeDaiLy = pvs.Where(c => c.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeDaiLy).Sum(c => c.GiaVeHienTai);
                        item.DoanhThuVeDiNgay = pvs.Where(c => c.vexeitem.LoaiVeId == (int)ENLoaiVeXeItem.VeDiNgay).Sum(c => c.GiaVeHienTai);
        ...
        #endregion
        #region
        public NhaXeCustomer GetKhachHang(string ThongTin)
        {
            return _khachhangRepository.Table.Where(c => c.HoTen == ThongTin || c.DienThoai == ThongTin).FirstOrDefault();
        }
        #endregion
        #region dai ly
        public virtual IPagedList<DaiLy> GetAllDaiLy(int NhaXeId = 0,
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            bool showHidden = false)
        {
            ...
            query = query.OrderBy(m => m.Id);
            return new PagedList<DaiLy>(query, pageIndex, pageSize);
        }
        public virtual List<DaiLy> GetAllDaiLy()
        {
            return _dailyRepository.Table.ToList();
        }
        public virtual void InsertDaiLy(DaiLy item)
        {
            _dailyRepository.Insert(item);
        }
        public virtual void UpdateDaiLy(DaiLy item)
        {
            _dailyRepository.Update(item);
        }
        public virtual void DeleteDaiLy(DaiLy item)
        {
            _dailyRepository.Delete(item);
        }
        public virtual DaiLy GetDaiLyById(int id)
        {
