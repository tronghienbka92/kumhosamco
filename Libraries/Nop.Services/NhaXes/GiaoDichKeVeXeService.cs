...
namespace Nop.Services.NhaXes
{
    public class GiaoDichKeVeXeService : IGiaoDichKeVeXeService
    {
        ...
        private readonly IRepository<GiaoDichKeVeLoaiVe> _giaodichkeveloaiveRepository;
        ...
        public GiaoDichKeVeXeService(
            ...
            IRepository<GiaoDichKeVeLoaiVe> giaodichkeveloaiveRepository,
            ...
            )
        {
            ...
            this._giaodichkeveloaiveRepository = giaodichkeveloaiveRepository;
        ...
        public virtual void InsertGiaoDichKeVeLoaiVe(GiaoDichKeVeLoaiVe item)
        {
            if (item == null)
                throw new ArgumentNullException("GiaoDichKeVeLoaiVe");
            _giaodichkeveloaiveRepository.Insert(item);
        }
        ...
        public virtual List<MenhGiaVe> GetAllMenhGia(int NhaXeID)
        {
            ...
            var query = _menhgiaveRepository.Table.Where(c => c.NhaXeId == NhaXeID && !c.isDelete).OrderBy(c=>c.MenhGia);
            return query.ToList();
        ...
        public virtual List<GiaoDichKeVeMenhGia> GetTonGiaoDichKeVeMenhGia(int NhanVienId, int MenhGiaVeId, bool isVeDi, int VanPhongId, int LoaiVeId,int HanhTrinhId,int ChangId)
        {
            ...
            if (ChangId > 0)
                query = query.Where(c => c.ChangId == ChangId);

            //query = query.OrderBy(t => t.MauVeKyHieuId).ThenBy(m => m.ThuTuBan).ThenBy(c => c.SoSeriNum);
            query = query.OrderBy(t => t.MauVeKyHieuId).ThenBy(c => c.SoSeriNum);
        ...
        {
            var query = _vexeitemRepository.Table.Where(c => c.XeXuatBenId == XeXuatBenId && !c.isDeleted && !c.isKhachHuy
                );
            return query.ToList();
        }
        /// <summary>
        ...
        public virtual List<VeXeItem> GetVeXeBanTrenXe(int NhaXeId, HistoryXeXuatBen chuyenxe)
        {
            ...
            var query = _vexeitemRepository.Table.Where(c => c.NhaXeId == NhaXeId && !c.isDeleted
                ...
                && (c.VanPhongId > 0)
                && (c.isVeBoSung)
                && (c.TrangThaiId == (int)ENVeXeItemTrangThai.DA_SU_DUNG || c.TrangThaiId==(int)ENVeXeItemTrangThai.DA_GAN_SERI_BOSUNG)
                && c.XeXuatBenId == chuyenxe.Id
                );
        ...
        public virtual void DeleteMauVe(QuanLyMauVeKyHieu item)
        {
            _quanlymauvekyhieuRepository.Delete(item);
        }
        public virtual List<QuanLyMauVeKyHieu> GetAllMauVeKyHieu(int NhaXeId)
        {
            if (NhaXeId == 0)
                return null;
            var query = _quanlymauvekyhieuRepository.Table.Where(c => c.NhaXeId == NhaXeId && !c.isDelete).OrderByDescending(o=>o.isMacDinh).ThenByDescending(m=>m.Id).ToList();
            return query;
        }
        ...
        public List<ChuyenDiTaiChinh> GetChuyenDiTaiChinhByVanPhongId(int VanPhongId, DateTime NgayDi)
        {
            var query = _chuyenditaichinhRepository.Table.Where(c => c.luotdi.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY
                && c.NgayTao.Year == NgayDi.Year
                && c.NgayTao.Month == NgayDi.Month
                && c.NgayTao.Day == NgayDi.Day);
            if(VanPhongId>0)
                query=query.Where(c=>c.nguoitao.VanPhongID==VanPhongId);
            return query.ToList();
        }
