...
namespace Nop.Services.NhaXes
{
    ...
    { 
        ...
        public static string ToMoTaHanhTrinhGiaVeThoiGian(this HanhTrinhGiaVeThoiGian item)
        {
            if (item == null)
                return "NULL";
            if (item.isDola)
                return string.Format("{0} - {1} ({2} đ)", item.DiemDi.TenDiemDon, item.DiemDen.TenDiemDon, item.GiaVe.ToDollar());
            return string.Format("{0} - {1} ({2} đ)", item.DiemDi.TenDiemDon, item.DiemDen.TenDiemDon, item.GiaVe.ToSoNguyen());
        }
