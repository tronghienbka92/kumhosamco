...
namespace Nop.Services.NhaXes
{
    public partial interface INhaXeService
    {
        ...
        List<HistoryXeXuatBen> GetChuyenDiByXeVanChuyenId(int XeId, DateTime NgayDi);
        ...
        #region khach hang
        NhaXeCustomer GetKhachHang(string ThongTin);
        #endregion
        #region dai ly
        IPagedList<DaiLy> GetAllDaiLy(int NhaXeId = 0,
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            bool showHidden = false);
        List<DaiLy> GetAllDaiLy();
        void InsertDaiLy(DaiLy item);
        void UpdateDaiLy(DaiLy item);
        void DeleteDaiLy(DaiLy item);
